...
from .rsi_triple_lines import RSITripleLinesSignalGenerator


class RSITripleLinesBacktester:
    """RSI三线策略回测引擎"""
    ...
    def run_backtest(self, etf_code: str, start_date: str = None,
        ...
        try:
            # 初始化信号生成器
            signal_generator = RSITripleLinesSignalGenerator(strategy_params)
    ...
    def _execute_trades(self, data: pd.DataFrame) -> Tuple[List[Dict], pd.DataFrame]:
        ...
        for idx, row in data.iterrows():
            ...
            performance_records.append({
                ...
                'rsi2': row.get('rsi2', 0),
                'rsi3': row.get('rsi3', 0),
                'triple_alignment': row.get('triple_alignment', ''),
                'rsi1_direction': row.get('rsi1_direction', ''),
    ...
    def _calculate_metrics(self, trades: List[Dict], performance: pd.DataFrame,
        ...
        # 统计金叉死叉次数
        golden_cross_count = len([t for t in trades if '金叉' in t.get('reason', '')])
        death_cross_count = len([t for t in trades if '死叉' in t.get('reason', '') or '清仓' in t.get('reason', '')])
