...
Optimizes per-ETF: MACD params + exhaustion_ratio + confirmation + smoothing + max_change.
...
class HistogramParamOptimizer:
    ...
    def __init__(self, etf_code: str, start_date: str = '20250101',
        ...
        # Bounds: [macd_fast, macd_slow, macd_signal, exhaustion_ratio, confirm_days, smooth, max_change]
        self.bounds = [
            (5, 26),      # macd_fast
            (13, 55),     # macd_slow
            (3, 13),      # macd_signal
            (0.3, 0.8),   # exhaustion_ratio
            (1, 5),       # confirm_days
            (1, 3),       # smooth
            (2, 10),      # max_change
        ]
    ...
    def _ensure_valid(self, params: dict) -> dict:
        ...
        params['deadzone'] = 0.0
        params['exhaustion_ratio'] = max(0.3, min(0.8, params.get('exhaustion_ratio', 0.5)))
    ...
    def _generate_signals(self, df: pd.DataFrame, params: dict) -> pd.DataFrame:
        generator = MACDHistogramMomentumSignalGenerator(params)
    ...
    def walk_forward_validate(self, params: dict = None, windows: List[Tuple[str, str, str]] = None) -> Dict:
        """Evaluate one parameter set across fixed train/validation date windows."""
        if params is None:
            params = MACDHistogramMomentumSignalGenerator.default_params()
        if windows is None:
            windows = [
                ('train_2020_2023', '20200101', '20231231'),
                ('validation_2024', '20240101', '20241231'),
                ('validation_2025_2026', '20250101', None),
            ]

        results = []
        for name, start_date, end_date in windows:
            data = get_etf_daily_data(self.etf_code, start_date=start_date, end_date=end_date)
            if not data or len(data) < 30:
                results.append({
                    ...
                    'end_date': end_date,
                    'success': False,
                    'message': f'数据不足: {len(data) if data else 0}天',
                    'metrics': {},
                })
                continue

            df = pd.DataFrame(data)
            _, metrics = self._evaluate_params(params, df)
            results.append({
                'name': name,
                'start_date': start_date,
                'end_date': end_date,
...
def batch_optimize(etf_codes: List[str] = None, start_date: str = '20250101',
    ...
    print(f"批量优化总结")
    print(f"{'='*70}")
    print(f"{'ETF':<15} {'收益%':<10} {'夏普':<8} {'胜率%':<8} {'交易':<8} {'MACD参数':<22} {'EXH':<8}")
    print("-" * 90)
    for code, r in results.items():
        ...
        print(f"{code:<15} {m['total_return_pct']:<10.2f} {m['sharpe_ratio']:<8.2f} "
              f"{m['win_rate_pct']:<8.1f} {m['total_trades']:<8} {macd_str:<22} "
              f"{p.get('exhaustion_ratio', 0.5):<8.2f}")
